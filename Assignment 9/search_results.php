...
// --- STEP 1: Establish DB connection
	...
	$sql = "SELECT dvd_title_id, dvd_titles.title AS title, dvd_titles.release_date AS date, genres.genre AS genre, ratings.rating AS rating
		FROM dvd_titles
		LEFT JOIN labels
						...
						<?php while($row = $results->fetch_assoc() ) : ?>
							<tr>
								<td>
									...
									<a href="delete.php?dvd_title_id=<?php echo $row['dvd_title_id']?>&title=<?php echo $row['title']?>" class="btn btn-outline-danger delete-btn">
										Delete
									</a>
								</td>
								<td>
